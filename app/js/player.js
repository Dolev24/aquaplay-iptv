/* player.js — one interface over two very different players.
...
(function (w) {
  ...
  function mediaErrText(e) {
    ...
    switch (e.code) {
      ...
      case 2: return 'Network error while streaming';
      ...
      case 4: return 'This browser cannot play this format (raw .ts needs the TV)';
  ...
  P.isDecodeError = function (msg) {
    return typeof msg === 'string' && msg.indexOf('decode') > -1;
  };
  ...
  P.applyPictureSize = function () {
    if (P.isTizen) { applyRect(); return; }   // the rect carries the shape there
  ...
  P.diag = function () {
    ...
    return {
      ...
      method: P.usedMethod || (P.isTizen ? '(none accepted)' : (videoEl ? videoEl.style.objectFit : '-')),
      state: state,
  ...
  function startTizen(url) {
    ...
    try {
      ...
      av.setListener({
        ...
        onstreamcompleted:   function () { fire('onError', 'Stream ended'); },
      ...
      av.prepareAsync(function () {
        ...
        P.settle();
        try { av.play(); } catch (e) { fire('onError', 'Could not start playback'); return; }
